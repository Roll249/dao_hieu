...
\emph{Experimental evidence.} Contrasting the \emph{w/o QAOA} row with the \emph{Full Quantum-HRL} row of Table~\ref{tab:ablation} isolates this addition: on the light-load base instance QAOA lowers mean latency by a modest $30.8\%$, consistent with the small $M_{l^*}\!\leq\!5$ search space; the effect is $+35\%$ under heavy load and $+164\%$ under full-day mobility (Section~\ref{subsec:ablation}). The depth sweep of Fig.~\ref{fig:depth} corroborates Proposition~\ref{prop:qaoa_convergence}: latency decreases with $p$ and saturates at $p\!=\!2$ for the considered $M_{l^*}$, confirming that the chosen depth is already at the diminishing-returns regime of the operator.
...
Fig.~\ref{fig:depth} sweeps $L\in\{1,2,4,6\}$ and $p\in\{1,2,3\}$. Consistent with the expressivity argument of Proposition~\ref{prop:qaoa_convergence}, increasing $p$ improves average latency up to $p\!=\!2$ and then saturates for the small per-tier problems considered here (we observe no further gain, not a guaranteed monotone law). The VQC depth shows typical bias--variance behaviour: too few layers under-fit, while $L\!\geq\!6$ yields no additional gain but doubles the gradient cost. The setting $L\!=\!4$, $p\!=\!2$ adopted throughout therefore constitutes a Pareto-efficient operating point.
...
The QAOA contribution should be read honestly and is strongly \emph{regime-dependent}. In the light-load base benchmark, disabling QAOA raises mean latency by only $+30.8\%$ (Table~\ref{tab:ablation}): the per-tier node-selection problem has at most $M_{l^*}\!\leq\!5$ candidate nodes, so even a random selector returns a feasible decision a non-trivial fraction of the time, and the QAOA approximation ratio at $p\!=\!2$ already saturates (Fig.~\ref{fig:depth}). Under the heavy-load regime the penalty is $+35\%$---essentially unchanged from the base regime, so congestion alone does not sharpen QAOA's contribution. The picture changes only under the full-day (24-hour) LuST mobility, where it reaches $+164\%$, because a diurnal mix of congested and sparse node populations rewards an informed choice across a far wider range of instances than a single stationary load level does. We therefore do not claim that QAOA confers a generic combinatorial advantage on the easy base instance; its value materialises precisely when the search space is loaded, and---crucially---it delivers that value at a \emph{constant} $2p$-parameter cost regardless of node count, whereas the classical node-selection head must grow (Section~\ref{subsec:scalability}). It is this constant footprint under scaling, not a mean-latency speed-up on small instances, that constitutes QAOA's contribution to the efficiency thesis.
