...
\begin{figure}[t]
  ...
  \caption{End-to-end Quantum-HRL system architecture for multi-layer NTN-VEC task offloading. The vehicular network state $s_t$ is collected and amplitude-encoded into a quantum register (left); the VQC-based higher-level agent outputs the tier selection $l^*$ and offloading ratio $\alpha$; the QAOA-based lower-level optimiser solves the node-selection problem to return $n^*$; the executed offloading action generates reward feedback, which drives the VQC policy-gradient update and the Bayesian-Optimisation refinement of the QAOA angles in the main learning loop.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{quantum_hrl_decision_flow.png}
  \caption{Quantum-HRL decision and learning flow. Vehicle state collection feeds the VQC policy network, whose high-level decision $(l^*,\alpha)$ conditions the QAOA-based low-level node selection $n^*$; the integrated action $a_t=(l^*,n^*,\alpha)$ is executed in the NTN-VEC environment, and the resulting reward $R_t$ and next state $s_{t+1}$ close the learning loop---updating the VQC parameters via the policy gradient and the QAOA angles via Bayesian Optimisation.}
...
This section specifies the simulation environment and dataset (Section~\ref{subsec:setup}), the hardware and software configuration (Section~\ref{subsec:config}), the experimental scenarios (Section~\ref{subsec:scenarios}), and the evaluation metrics (Section~\ref{subsec:metrics}). The same data-input, processing, decision, and output pipeline is applied to every method under comparison; only the high-level/low-level policy is swapped, isolating the contribution of the proposed framework.

\subsection{Dataset and Simulation Environment}
\label{subsec:setup}

\paragraph{Network topology and mobility.}
Following the topology of Shinde and Tarchi~\cite{hrl_ntn}, we instantiate a four-tier T-NTN over a $5$-km urban road network with a single fallback Cloud facility~$\mathcal{C}$. Tier altitudes are taken from~\cite[Table~3]{hrl_ntn}: LAPs at $h\!\approx\!1\,$km, HAPs at $h\!\approx\!100\,$km, and LEO satellites at $h\!\approx\!2000\,$km, with RSU coverage $50\,$m, LAP coverage $200\,$m, and HAP coverage $1\,000\,$m. To enable a deterministic action space for the high-level VQC head we fix the per-tier deployment to $M_1{=}5$ RSUs, $M_2{=}3$ LAPs, $M_3{=}2$ HAPs, and $M_4{=}2$ LEO satellites (the baseline of~\cite{hrl_ntn} instantiates node counts dynamically from coverage radius and road length; our fixed choice is a moderate extension held constant across all compared methods). Vehicular users follow the speed model of~\cite[Table~3]{hrl_ntn}, drawn from $\mathcal{U}(8,14)\,$m/s, conditioned on the road segment.

\paragraph{Task and traffic model.}
At each slot $\tau_i$, every vehicle $k$ generates an independent task $x_k=(d_k,c_k,T_k^{\max},\xi_k)$. Whereas~\cite{hrl_ntn} uses a single nominal task profile, we extend their setup to a heterogeneous workload by sampling $d_k\!\sim\!\mathcal{U}(0.5,5)\,$Mbytes, $c_k\!\sim\!\mathcal{U}(100,1000)\,$Mcycles, and $T_k^{\max}\!\sim\!\mathcal{U}(0.5,2)\,$s, stressing the VQC's ability to react to heterogeneous demand. The service type is sampled uniformly from $|\Xi|{=}6$ classes as in~\cite{hrl_ntn}; each EN supports a fixed random subset $\Xi_e\subseteq\Xi$ that mandates cloud relay whenever $\xi_k\notin\Xi_e$, captured by $b_e^{\xi_k}$.

\paragraph{Communication parameters.}
The path-loss reference $\eta_0$, the exponent $\delta$, and the per-tier bandwidth $B_{k,e}$ follow~\cite{hrl_ntn}; transmit power, noise spectral density, and EN CPU frequencies are likewise taken from that reference, preserving inter-tier disparities (e.g.\ LEO latency floors, HAP coverage radius).

\paragraph{State assembly.}
Per slot, the simulator (i)~advances each VU's position $(\mathbf{r}_k^t,\mathbf{v}_k^t)$ and recomputes sojourn times $T_{v_k,\mathrm{soj}}^{e}(i)$; (ii)~samples tasks $x_k$; (iii)~recomputes path-loss gains $\{h_{k,e}^t\}$ via Eq.~\eqref{eq:channel_gain}; (iv)~reads each EN's CPU utilisation $\{u_{l,n}^t\}$ and queue length; and (v)~concatenates these into the raw state $s_t\in\R^{20}$, normalised to $\tilde s_t = s_t/\norm{s_t}_2$ to satisfy the amplitude-encoding precondition (Corollary~\ref{cor:norm}).

\subsection{Hardware and Software Configuration}
\label{subsec:config}

\paragraph{Software stack and quantum back-end.}
The simulator is implemented in Python on NumPy for the classical environment and a PennyLane statevector simulator for the VQC and QAOA primitives; inner training loops use analytic (exact) expectation values for tractable turnaround, while finite-shot effects ($N_{\mathrm{shots}}{=}1024$ per observable) and hardware-style Gaussian readout noise are studied separately in the NISQ-noise scenario (Section~\ref{subsec:scenarios}). The Bayesian Optimiser uses a Mat\'ern-$5/2$ Gaussian-process surrogate (scikit-learn) with Expected-Improvement acquisition~\cite{bo_qaoa}. A complete five-seed training-plus-evaluation run with the default budget (including the ablation sweep) completes in under one hour on a single commodity workstation without a GPU. The codebase, configuration files, and random seeds will be released upon publication.
...
\begin{description}
  \item[Scenario 1 --- Classical HRL.] \emph{Motivation:} establish the learned classical reference under the exact same environment and feasibility constraints. \emph{Purpose:} quantify the offloading quality and parameter footprint attainable by the tri-DQN decomposition of~\cite{hrl_ntn}. \emph{Expected outcome:} feasible offloading (zero violations after convergence) at an $O(n\cdot h)$ parameter cost, serving as the baseline against which all quantum components are measured.
  \item[Scenario 2 --- HRL + VQC.] \emph{Motivation:} test whether replacing the high-level (tier/ratio) DQNs with a single amplitude-encoded VQC preserves decision quality at a logarithmic parameter cost. \emph{Purpose:} isolate the contribution of the VQC high-level policy while node selection remains non-quantum. \emph{Expected outcome:} comparable or improved tier/ratio decisions with a drastic reduction in high-level parameters; because tier and ratio carry most of the decision value, this step should account for the bulk of the latency change.
  \item[Scenario 3 --- VQC + QAOA.] \emph{Motivation:} test whether replacing the flat $Q$-table node selector with the structure-aware QAOA solver reduces both the mean and the variance of latency. \emph{Purpose:} isolate the contribution of QAOA-based combinatorial node selection on top of the VQC high-level policy. \emph{Expected outcome:} lower-variance node decisions that avoid overloaded or distant nodes, tightening the latency distribution relative to a random or tabular selector.
  \item[Scenario 4 --- Full Quantum-HRL.] \emph{Motivation:} evaluate the complete framework with the Bayesian-Optimisation outer loop tuning the QAOA angles under measurement noise. \emph{Purpose:} measure end-to-end offloading quality, the total parameter footprint, and training stability. \emph{Expected outcome:} the lowest mean latency among all configurations at the smallest parameter count, with the latency--energy trade-off reported transparently.
\end{description}
...
The study uses task-level, learning-level, and efficiency metrics. \emph{Task-level:} average task latency $\bar T=\mathbb{E}[T^{x_k}]$ (s) and energy $\bar E=\mathbb{E}[E^{x_k}]$ (J); deadline-miss rate $\Pr(F^2{=}1)$, sojourn-violation rate $\Pr(F^1{=}1)$, and energy-suboptimality rate $\Pr(F^3{=}1)$, all in $\%$, assessing feasibility under (C2)--(C4). \emph{Learning behaviour:} cumulative reward $\sum_t \gamma_{\mathrm{MDP}}^t R_1^t$ versus episode; episodes-to-plateau $\Pi_\eta$ (episodes until the moving-average reward reaches $\eta\%$ of its asymptote, $\eta{=}95$); and seed-wise variance of the converged reward. \emph{Model efficiency:} trainable-parameter count $W$ and reduction factor $\rho=W_{\mathrm{HRL}}/W_{\mathrm{Q\text{-}HRL}}$; circuit evaluations per update (the dominant NISQ cost); and wall-clock time per episode. Table~\ref{tab:metrics_summary} consolidates the definitions and optimisation directions so that every numerical entry in Section~\ref{sec:results} is unambiguous.
...
Findings are obtained on a four-tier T-NTN with fixed, modest per-tier node counts, synthetic task arrivals, and a path-loss channel model inherited from~\cite{hrl_ntn}. Generalisation to larger networks, real mobility traces, correlated traffic, or richer channel models is not established here; the scalability scenario probes only $\zeta\in\{1,2,4\}$, and physical NISQ qubit budgets would become binding for large tiers. The absolute latency and energy values are simulator-specific and should be read comparatively, not as deployment figures.
...
All seeds, hyperparameters, and the deterministic mobility/task generators are documented, and a full five-seed run completes in under an hour on a commodity CPU; the code, configurations, and seeds will be released upon publication. The chief reproducibility caveat is the use of analytic (noiseless) expectation values in the inner loop for tractability; finite-shot and Gaussian-noise effects are confined to the NISQ-noise scenario, so results on physical hardware may differ and should be confirmed by the hardware-deployment study identified as future work.
...
\paragraph{Summary.}
We presented \textbf{Quantum-HRL}, a hybrid quantum--classical hierarchical reinforcement learning framework for adaptive task offloading in multi-layer T-NTN vehicular edge computing. It replaces the classical tri-DQN pipeline of~\cite{hrl_ntn} with a VQC-driven high-level policy (tier and ratio) and a QAOA-based combinatorial node selector tuned through Bayesian Optimisation, coupled through a single MDP loop and trained with advantage-weighted Parameter-Shift-Rule policy gradients for the VQC and Bayesian Optimisation for the QAOA angles.

\paragraph{Main findings.}
On the five-seed T-NTN benchmark, Quantum-HRL delivers $20.3\%$ lower mean task latency than the classical HRL baseline ($0.117\pm0.038$ vs.\ $0.147\pm0.016$\,s; $p<10^{-3}$ under both Welch's $t$ and Mann--Whitney $U$ tests) with zero deadline, sojourn, or energy-budget violations, while reducing the trainable-parameter count from $20{,}224$ to $24$---an $843\times$ compression that reflects a change in scaling from $O(n\cdot h)$ to $O(L\log_2 n + p)$. Ablation confirms that both quantum modules contribute: removing QAOA doubles latency and inflates its variance, and removing the VQC degrades latency by more than an order of magnitude. The gain comes at a transparently reported higher per-task energy ($0.97$ vs.\ $0.35$\,J), a steerable latency--energy trade-off governed by the reward weights.

\paragraph{Limitations.}
The evaluation is simulation-only and uses a statevector back-end rather than real quantum hardware; the workloads are synthetic and independently sampled; the deployment uses fixed, modest per-tier node counts with simplified mobility and channel models; and the inner training loop uses noiseless expectation values, with hardware noise confined to a dedicated scenario. The framework also inherits NISQ-era constraints (barren plateaus at large depth, finite qubit budgets) that bound how far it can scale today.

\paragraph{Future work.}
Future research will (a)~deploy the full pipeline on real superconducting and trapped-ion back-ends to test whether the simulated noise model is conservative or optimistic; (b)~adopt hardware-efficient, topology-matched ans\"atze and data re-uploading layers to mitigate barren-plateau effects at modest qubit cost; (c)~extend the action space to multi-task concurrent offloading with task dependencies, e.g.\ via a quantum actor--critic decomposition; (d)~explore federated quantum learning across vehicular fleets, exploiting the small parameter footprint reported here; and (e)~pursue formal quantum--classical separation results for the QUBO instances arising from T-NTN offloading, to place the empirical advantages on a stronger theoretical footing.
