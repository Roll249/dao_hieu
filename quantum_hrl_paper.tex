...
\emph{Experimental evidence.} Contrasting the \emph{w/o QAOA} row with the \emph{Full Quantum-HRL} row of Table~\ref{tab:ablation} isolates this addition: on the light-load base instance QAOA lowers mean latency by a modest $31\%$, consistent with the small $M_{l^*}\!\leq\!5$ search space, and its contribution grows to $+35\%$ under heavy load and $+164\%$ under full-day mobility (Section~\ref{subsec:ablation}). The depth sweep of Fig.~\ref{fig:depth} corroborates Proposition~\ref{prop:qaoa_convergence}: latency decreases with $p$ and saturates at $p\!=\!2$ for the considered $M_{l^*}$, confirming that the chosen depth is already at the diminishing-returns regime of the operator.
...
The QAOA contribution should be read honestly and is strongly \emph{regime-dependent}. In the light-load base benchmark, disabling QAOA raises mean latency by only $+31\%$ (Table~\ref{tab:ablation}): the per-tier node-selection problem has at most $M_{l^*}\!\leq\!5$ candidate nodes, so even a random selector returns a feasible decision a non-trivial fraction of the time, and the QAOA approximation ratio at $p\!=\!2$ already saturates (Fig.~\ref{fig:depth}). The picture changes once node selection becomes a binding combinatorial decision: under the heavy-load regime the penalty for removing QAOA grows to $+35\%$, and under the full-day (24-hour) LuST mobility to $+164\%$ (mean latency roughly triples), because congested, heterogeneous node populations reward an informed choice. We therefore do not claim that QAOA confers a generic combinatorial advantage on the easy base instance; its value materialises precisely when the search space is loaded, and---crucially---it delivers that value at a \emph{constant} $2p$-parameter cost regardless of node count, whereas the classical node-selection head must grow (Section~\ref{subsec:scalability}). It is this constant footprint under scaling, not a mean-latency speed-up on small instances, that constitutes QAOA's contribution to the efficiency thesis.
...
\item \textbf{Training stability of policy-gradient learning.} The VQC is trained by high-variance parameter-shift REINFORCE gradients. Across ten seeds we observed one training collapse (final latency $2.24$\,s at $20.8\%$ deadline miss on that seed), which inflates the mean and variance (Section~\ref{subsec:main}). We attribute this to policy-gradient learning rather than to the quantum circuit: the parameter-matched classical REINFORCE control collapses \emph{more} often ($4/10$ seeds), and only the temporal-difference tri-DQN avoids collapse entirely. Deployment of any REINFORCE-trained policy here would therefore benefit from variance-reduction (baselines, entanglement-aware initialisation, layer-wise pre-training) or seed selection against a held-out validation check. The quantum policy's collapse, though rarer, is deeper than the classical control's, so its worst-case tail is heavier.
...
\item \textbf{Light-load, no-queue regime.} Edge-node CPU load is tracked as a bounded exponential-moving-average utilisation and enters the state vector, but the latency model adds \emph{no} explicit queueing/waiting term ($T_w\!\approx\!0$): the reported results characterise a light-load operating point in which contention does not dominate. A congestion-heavy regime with an explicit queue would be needed to substantiate load-aware claims and is left to future work.
