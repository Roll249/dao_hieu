...
\emph{Experimental evidence.} Table~\ref{tab:results} records the classical HRL endpoint: mean per-task latency of $0.147\pm0.016$\,s, energy of $0.347\pm0.095$\,J, zero miss rate, and $20{,}224$ trainable parameters. Fig.~\ref{fig:convergence} shows the corresponding reward trajectory: monotone but slow, with the wider inter-seed band typical of deep $Q$-learning on a $\sim$$2\!\times\!10^{4}$-parameter network.
...
\emph{Newly introduced component.} An amplitude-encoded Variational Quantum Circuit of depth $L\!=\!4$ that jointly produces the tier and ratio actions, trained by advantage-weighted Parameter-Shift-Rule policy gradients (Section~\ref{subsec:vqc}). The node-selection head remains a non-quantum selector at this stage.
...
\emph{Experimental evidence.} The corresponding configuration in the ablation table (\emph{w/o QAOA}, Table~\ref{tab:ablation}) and the parameter accounting in Table~\ref{tab:complexity} jointly characterise Stage~II: with $W_{\mathrm{VQC}}\!=\!L\lceil\log_2 n\rceil\!=\!20$ parameters the VQC keeps the high-level decision quality within reach of the classical baseline, while the reward trajectory of Fig.~\ref{fig:convergence} confirms the faster, lower-variance convergence predicted by hypothesis~(iii).
\emph{Conclusion of this stage.} The VQC validates the parameter-efficiency claim and accounts for the bulk of the convergence-speed improvement, but the node-selection head is still a flat selector unaware of the combinatorial structure of the assignment. Latency variance therefore remains dominated by occasional excursions onto overloaded or distant nodes, motivating Stage~III.

\paragraph{Stage~III --- HRL + VQC + QAOA.}
\emph{Motivation.} Node selection is a structured combinatorial problem over at most $M_{l^*}\!\leq\!6$ candidates of the chosen tier, with a one-hot feasibility constraint and a per-node cost determined by the current state. A flat selector ignores this algebra and can produce heavy-tailed latencies, while a structure-aware solver can exploit the QUBO form directly.
...
\emph{Research hypothesis.} QAOA should reduce both the mean and the variance of node-selection latency by biasing the wavefunction toward the QUBO ground state, with the largest gain expected on the most loaded tiers; because the per-tier search space is small ($M_{l^*}\!\leq\!6$), only a modest mean-latency improvement is anticipated, while the tail of the latency distribution should tighten markedly.
\emph{Experimental evidence.} Contrasting the \emph{w/o QAOA} row with the \emph{Full Quantum-HRL} row of Table~\ref{tab:ablation} isolates this addition: replacing the structure-blind selector by QAOA halves the mean latency and shrinks its inter-seed standard deviation by roughly $7\times$. The depth sweep of Fig.~\ref{fig:depth} corroborates Proposition~\ref{prop:qaoa_convergence}: latency decreases monotonically with $p$ and saturates at $p\!=\!2$ for the considered $M_{l^*}$, confirming that the chosen depth is already at the diminishing-returns regime of the operator.
\emph{Conclusion of this stage.} QAOA delivers the predicted variance reduction at constant-depth circuit cost, but its angles are fixed at deployment. In a NISQ setting these angles are sensitive to readout noise and to per-instance cost variability, so a single static $(\gamma,\beta)$ choice is unlikely to remain optimal across the encountered QUBO instances, motivating Stage~IV.
...
\emph{Experimental evidence.} Table~\ref{tab:results} reports the integrated Full Quantum-HRL endpoint---mean latency $0.117\pm0.038$\,s, zero miss rate, $24$ trainable parameters---the lowest mean latency and smallest footprint among all evaluated methods, validated by Welch's $t$ and Mann--Whitney $U$ tests at $p<10^{-3}$. The convergence trajectory in Fig.~\ref{fig:convergence} exhibits the tightest inter-seed band of all configurations, and the NISQ-noise study (Section~\ref{subsec:scalability}) shows that BO maintains reward stability up to noise levels an order of magnitude larger than those reported on current superconducting back-ends~\cite{preskill2018nisq}, consistent with Proposition~\ref{prop:bo_efficiency}.
...
Disabling the VQC and reverting to a random tier/ratio policy (row \emph{w/o VQC} of Table~\ref{tab:ablation}) degrades the average latency by $+1{,}763\%$ to $2.009\pm0.094$\,s---essentially the latency of a random policy. The magnitude of this drop quantifies the value of high-level decision-making: tier and ratio selection determine \emph{where} the workload is sent and \emph{how aggressively} it is offloaded, so a single ill-chosen action affects every subsequent slot through queueing and sojourn-time feedback. The VQC therefore accounts for the bulk of the structural decision quality validated in Stage~II of Section~\ref{subsec:scenarios}; the $5$-qubit amplitude encoding is sufficient to represent the load--mobility couplings driving these decisions, despite carrying only $W_{\mathrm{VQC}}=20$ trainable parameters.

\paragraph{Contribution of the QAOA solver.}
Replacing the QAOA node selector with a structure-blind random selector (row \emph{w/o QAOA} of Table~\ref{tab:ablation}) approximately doubles the mean latency ($+105.5\%$) and inflates its inter-seed standard deviation by roughly $7\times$. The variance amplification is more diagnostic than the mean shift: QAOA's benefit is concentrated in the \emph{tail} of the latency distribution---it avoids the occasional excursions onto overloaded or distant nodes that drive deadline violations. The mean-latency contribution is comparatively modest because the per-tier search space is small ($M_{l^*}\!\leq\!6$ candidates), so a random selector recovers a feasible decision a non-trivial fraction of the time. The ablation thus confirms the prediction stated for Stage~III: under a small node-selection search space, QAOA primarily tightens the tail rather than collapsing the mean.

\paragraph{Contribution of Bayesian Optimisation.}
BO does not appear in Table~\ref{tab:ablation} as a separate row because removing it does not alter the trainable-parameter count---BO acts on the $2p$ QAOA angles already counted in $W_{\mathrm{Q\text{-}HRL}}$---but its contribution is observable in three complementary measurements. First, in the convergence trajectory of Fig.~\ref{fig:convergence}, the Full Quantum-HRL curve exhibits the tightest inter-seed band, consistent with BO stabilising the QAOA angles across initial conditions. Second, in the NISQ-noise scenario (Section~\ref{subsec:scalability}), the BO outer loop preserves reward stability up to gate-noise levels an order of magnitude larger than those reported on current superconducting devices~\cite{preskill2018nisq}, whereas a fixed-angle QAOA degrades monotonically with noise (Proposition~\ref{prop:bo_efficiency}). Third, by adapting angles online, BO converts Stage~III's static $(\gamma,\beta)$ choice into an instance-conditional one, which is precisely the failure mode that Stage~IV was introduced to address. BO's contribution is therefore best characterised as \emph{stability and noise robustness}, rather than a raw mean-latency reduction---a distinction that mirrors the Stage~III\,$\rightarrow$\,Stage~IV motivation in Section~\ref{subsec:scenarios}.
...
Three structural properties of the amplitude-encoded VQC explain its central role in the latency improvement. \emph{Information-dense encoding.} Amplitude encoding maps the full $n$-dimensional state into $\lceil\log_2 n\rceil$ qubits, so a single VQC observable already aggregates information from every classical feature (Proposition~\ref{prop:compression}); the policy can therefore react to cross-feature interactions---e.g.\ load--mobility couplings---that the per-DQN baseline must learn implicitly through wide hidden layers. \emph{Well-conditioned optimisation landscape.} Policy gradients are computed over a $20$-parameter manifold instead of $\sim$$2\!\times\!10^{4}$ weights, so updates are inherently better-conditioned and inter-seed variance is reduced (Fig.~\ref{fig:convergence}). \emph{Shared representation for hierarchical actions.} The single VQC head jointly produces the tier and ratio actions, so the two strongly correlated sub-decisions share representations rather than being learned by two independently trained networks. The VQC is consequently the source of most of the structural decision quality, as the ablation magnitude under \emph{w/o VQC} confirms. We caution that the well-conditioned-landscape and shared-representation effects are partly low-dimensionality and architectural in nature, and that a comparably small classical policy might reproduce them in part; isolating the strictly quantum component would require a parameter-matched classical control, which we leave to future work.

\paragraph{Why Bayesian Optimisation contributes the largest robustness improvement.}
BO operates on the $2p$ QAOA angles and contributes no trainable parameters, yet it is the single change that converts QAOA from a static, noise-sensitive solver into an adaptive one. Two properties account for its disproportionate impact. \emph{Sample efficiency on a low-dimensional landscape.} The QAOA cost surface $C(\gamma,\beta)$ is two- to four-dimensional, smooth, and bounded; a Mat\'ern-$5/2$ GP with EI acquisition is therefore near-optimal on it, and BO can locate the angle minimiser within $B\!=\!20$ queries (Proposition~\ref{prop:bo_efficiency}). \emph{Implicit averaging over noise.} The GP surrogate filters i.i.d.\ measurement noise through its kernel-weighted posterior, so BO continues to converge under finite-shot expectation estimates whereas a fixed-angle QAOA degrades monotonically. The combined effect is the tightest inter-seed convergence band among all evaluated configurations (Fig.~\ref{fig:convergence}) and the robustness window observed in the NISQ-noise scenario (Section~\ref{subsec:scalability}). BO is therefore the module that elevates the framework from a competitive classical alternative to a noise-tolerant operating point---a benefit that is invisible in any single-shot mean-latency number but appears clearly under variance and noise stress, and it is precisely this property that motivates the Stage~III\,$\rightarrow$\,Stage~IV transition.

\paragraph{Why QAOA provides only modest mean-latency gains.}
The QAOA contribution should be read honestly: although disabling QAOA roughly doubles the mean latency and inflates its variance by $\sim$$7\times$ (Table~\ref{tab:ablation}), the improvement attributable to QAOA when the rest of the pipeline is intact is dominated by the variance term rather than by the mean. The reason is structural rather than algorithmic: the per-tier node-selection problem has at most $M_{l^*}\!\leq\!6$ candidate nodes, so even a random selector occasionally returns a feasible decision, and the QAOA approximation ratio at $p\!=\!2$ already saturates (Fig.~\ref{fig:depth}). Under such a small combinatorial search space, the QUBO landscape simply does not contain enough exploitable structure for a quantum solver to deliver an order-of-magnitude mean-latency speed-up; what it can deliver---and does---is consistent avoidance of the worst nodes, which the ablation captures as variance reduction. We therefore do not claim that QAOA confers a generic combinatorial advantage on this problem; its role here is \emph{tail control}. We expect QAOA's mean-latency contribution to grow on deployments with larger per-tier node counts, where the search space is closer to the regime in which the QAOA approximation ratio becomes the binding factor.

\paragraph{Why per-task energy consumption increases.}
The higher per-task energy is a direct consequence of the latency-minimising policy, not an inefficiency of the framework. The VQC ratio policy converges to aggressive offloading ($\alpha\!\approx\!0.8$--$0.97$), shrinking the local branch of $\max(T_{\mathrm{off}},T_{\mathrm{loc}})$ in Eq.~\eqref{eq:latency} to near zero but paying the transmission energy $E_{tx}+E_{rx}$ and edge-compute energy $E_{c,ke}$ for nearly the entire workload. The classical baseline settles on a conservative split that keeps energy low at a latency cost. Both operating points satisfy all three feasibility constraints; they are different Pareto points on the same trade-off front, selected by the two learners' inductive biases.

\paragraph{Latency--energy trade-off.}
The latency gain therefore reflects a transparent latency--energy trade-off ($0.117$\,s at $0.971$\,J versus $0.147$\,s at $0.347$\,J). Because the operating point is governed by the reward weights, practitioners can steer Quantum-HRL toward the energy-lean end of the front simply by increasing $\beta_2$ in Eq.~\eqref{eq:reward}; the framework is agnostic to the chosen balance. There is also a compute trade-off: each VQC update costs $2W_{\mathrm{VQC}}$ circuit evaluations and each BO step a GP regression, so on classical simulators the wall-clock per episode is comparable to a plain DQN. The qualitative advantage is that model size and circuit count scale logarithmically with the problem dimension rather than linearly.
